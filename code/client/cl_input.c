...
void CL_KeyMove( usercmd_t *cmd ) {
	...
	int conditionFlag = cl.snap.ps.stats[STAT_BIG_MAC];
//	int 		weaponCount;

//	for (i = 0; i < 9; i++) { // Loop through 9 weapon slots
  //  				if (cl.snap.ps.stats[STAT_WEAPONS] & (1 << i))
    //    			weaponCount++;
	//			}
...
if (conditionFlag ==1) {  // If STAT_BIG_MAC is 1 (Armor > 40 OR Weapons > 4)
        movespeed = 108;   
    } else {
        movespeed = 127;
    }
		...
		cmd->buttons |= BUTTON_WALKING;		//	cmd->buttons &= ~BUTTON_WALKING;
...
//		Com_Printf("Condition Flag: %d\n", conditionFlag);

	forward = 0;
	side = 0;
	up = 0;
	if ( in_strafe.active ) {
		side += 88 * CL_KeyState (&in_right);
		side -= 88 * CL_KeyState (&in_left);
	}

	side += 88 * CL_KeyState (&in_moveright);
	side -= 88 * CL_KeyState (&in_moveleft);    // EUGEN --TEST am adaugat * 0.2


	up += 108 * CL_KeyState (&in_up);
	up -= 108 * CL_KeyState (&in_down);
