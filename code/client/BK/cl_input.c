...
void CL_KeyMove( usercmd_t *cmd ) {
	...
	int weaponCountz = cg.snap->ps.stats[STAT_NUMWEAPONS];   // Read stored weapon count
	int armorz = cg.snap->ps.stats[STAT_VALARMOR]; 
	...
	if ( in_speed.active ^ cl_run->integer ) {
		...
		movespeed = 127;
		G_Printf("Player Speed: %d\n", movespeed);  // EUGEN debug value for speed
