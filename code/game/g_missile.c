...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	if (other->client) {
	        int armura = other->client->ps.stats[STAT_ARMOR];
        	Com_Printf("Enemy Armor on impact: %d\n", armura); 
		if (armura > 40) {
            		int extraDamage = (rand() % (armura + 1)) / 2;  // Random bonus damage
    			ent->damage += extraDamage;
        	}
	}
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	bolt->damage = 100;
	bolt->splashDamage = 100;
	bolt->splashRadius = 240;	// default 120  
