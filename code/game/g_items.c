...
#define	RESPAWN_ARMOR		120
#define	RESPAWN_HEALTH		60
#define	RESPAWN_AMMO		48
#define	RESPAWN_HOLDABLE	160
#define	RESPAWN_MEGAHEALTH	3500//12000
#define	RESPAWN_POWERUP		360
...
void Add_Ammo (gentity_t *ent, int weapon, int count)
{
	...
	if ( ent->client->ps.ammo[WP_ROCKET_LAUNCHER] > 14 ) {
		ent->client->ps.ammo[WP_ROCKET_LAUNCHER] = 14;
	}
	if ( ent->client->ps.ammo[WP_PLASMAGUN] > 60) {
		ent->client->ps.ammo[WP_PLASMAGUN] = 60;	
	}

	if ( ent->client->ps.ammo[WP_MACHINEGUN] > 40) {
		ent->client->ps.ammo[WP_MACHINEGUN] = 40;	
	}

	if ( ent->client->ps.ammo[WP_RAILGUN] > 1 ) {
		ent->client->ps.ammo[WP_RAILGUN] = 1;
	}
...
int Pickup_Weapon (gentity_t *ent, gentity_t *other) {
	...
	if ( ent->count < 0 ) {
		...
		if ( ! (ent->flags & FL_DROPPED_ITEM) && g_gametype.integer != GT_TEAM ) {
			...
			if ( other->client->ps.ammo[ ent->item->giTag ] < quantity ) {
				...
				quantity = 0;		// only add a single shot  // EUGEN  -default 1
			}
		}
	}
	...
	// add the weapon
	  ...
	  // If it's the railgun, set its ammo to 0 and only allow 1 slug per pickup
						//   if (ent->item->giTag == WP_RAILGUN) {
//        other->client->ps.ammo[ent->item->giTag] = 0;  // Set railgun ammo to 0
						 //       quantity = 1; } // Limit slugs to 1 per pickup
//    } else {
        // Handle ammo for other weapons
//        quantity = ent->item->quantity;
//    }
...
//	else {
	...
	Add_Ammo( other, ent->item->giTag, quantity );    //}   //EUGEN  am bagat in bucla cu dat 0 pt railgun
...
//	}
	...
	if ( respawn <= 0 ) {
		...
		ent->nextthink = level.time + respawn * 1000;;				//  respawn * 1000;
