...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	VectorMA (muzzle, 140, forward, end);
...
#endif
	missingHealth = ent->client->ps.stats[STAT_MAX_HEALTH] - ent->health;
	damage = (75 + missingHealth) * s_quadFactor;
...
#endif
#define MACHINEGUN_SPREAD	400
#define	MACHINEGUN_DAMAGE	18
...
#define	DEFAULT_SHOTGUN_DAMAGE	30
...
void ShotgunPattern( vec3_t origin, vec3_t origin2, int seed, gentity_t *ent ) {
	...
	for ( i = 0 ; i < DEFAULT_SHOTGUN_COUNT ; i++ ) {
		r = Q_crandom( &seed ) * DEFAULT_SHOTGUN_SPREAD * 8;
		u = Q_crandom( &seed ) * DEFAULT_SHOTGUN_SPREAD * 8;
		VectorMA( origin, 2048 * 8, forward, end);
		VectorMA (end, r, right, end);
		VectorMA (end, u, up, end);
...
void weapon_supershotgun_fire (gentity_t *ent) {
	...
	VectorScale( forward, 1024, tent->s.origin2 );
	SnapVector( tent->s.origin2 );
	tent->s.eventParm = rand() & 128;		// seed for spread pattern
	tent->s.otherEntityNum = ent->s.number;
...
void Weapon_RocketLauncher_Fire (gentity_t *ent) {
	...
	m = fire_rocket (ent, muzzle, forward);
	 ...
	 // Modify damage based on target's armor
		if (m->enemy && m->enemy->client) {
        	armura = m->enemy->client->ps.stats[STAT_ARMOR]; 
			Com_Printf("Enemy Armor: %d\n", armura);
                	if (armura > 40) {
	            	int extraDamage = (rand() % (armura + 1)) / 4; // Random between 1 and armor, divided by 2
	            	m->damage += extraDamage;
        		}
    		}
	...
	m->damage = (m->damage + 40) * s_quadFactor;
	//EUGEN  aici sa adaug if armura MAI MARE DECAT... 40 ceva
	}
	m->splashDamage = (m->splashDamage - 40) * s_quadFactor;
...
void FireWeapon( gentity_t *ent ) {
	...
	case WP_MACHINEGUN:
				if ( g_gametype.integer != GT_TEAM ) {
					...
					// 1 in 5 bullets will have random damage between -140 and 140
					dmg = (rand() % 26) - 8; // Default damage FACUT RANDOM
					
					if (rand() % 4 == 0) { // 1 in 4 chance something
					    dmg = (rand() % 180); // Random damage between -140 and 140
					}

			Com_Printf("Machinegun Damage: %d\n", dmg, MOD_MACHINEGUN);
			Bullet_Fire( ent, MACHINEGUN_SPREAD, dmg, MOD_MACHINEGUN );
		} else {
