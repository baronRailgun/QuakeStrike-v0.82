...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time) {
	...
	if (bi->actionflags & ACTION_MOVEFORWARD) ucmd->forwardmove = 98;
	if (bi->actionflags & ACTION_MOVEBACK) ucmd->forwardmove = -98;
	if (bi->actionflags & ACTION_MOVELEFT) ucmd->rightmove = -114;
	if (bi->actionflags & ACTION_MOVERIGHT) ucmd->rightmove = 114;
	//jump/moveup
