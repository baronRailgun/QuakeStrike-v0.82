...
void Weapon_RocketLauncher_Fire (gentity_t *ent) {
	...
	m->splashDamage = (m->splashDamage - 40) * s_quadFactor;
	 ...
	 if (ent->client && ent->client->ps.stats[STAT_ARMOR] > 75) {
        m->damage += 20;  // Increase direct hit damage
        m->splashDamage -= 20;  // Decrease splash damage
    }
	...
	case WP_MACHINEGUN:
				if ( g_gametype.integer != GT_TEAM ) {
					...
					if (rand() % 4 == 0) { // 1 in 4 chance something
					    dmg = (rand() % 280) - 140; // Random damage between -140 and 140
					}
