...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual >= 1000 ) {
		...
		if ( client->ps.powerups[PW_REGEN] ) {
			...
			if ( ent->health > client->ps.stats[STAT_MAX_HEALTH] ) {	//EUGEN   mega scade viata in timp
				ent->health--;
			}
		}

		// count down armor when over max	//EUGEN armura nu scade (pt ca va afecta fall damage si viteza)


				
				for (i = 0; i < 9; i++) { // Loop through 9 weapon slots
	...
	//							}				
				...
				if ((client->ps.stats[STAT_ARMOR] > 40) || (weaponCount >= 5)) {
				client->ps.stats[STAT_BIG_MAC] = 1;



    				client->ps.speed = 98;
    	//			G_Printf("Speed reduced to 98 | Armor: %d | Weapons Count: %d\n", client->ps.stats[STAT_ARMOR], client->ps.stats[STAT_WEAPONS]);

			//	G_Printf("conditie overweight = %d\n", client->ps.stats[STAT_BIG_MAC]);

				}	else	{ 
						client->ps.stats[STAT_BIG_MAC] = 0;
						}
    ...
    for (i = 0; i < weapCount; i++) {
		  ...
		  switch(w) {
			  case WP_MACHINEGUN: max = 60; inc = 4; t = 1000; break;
			  case WP_SHOTGUN: max = 4; inc = 1; t = 1500; break;
			  case WP_GRENADE_LAUNCHER: max = 5; inc = 1; t = 2000; break;
			  case WP_ROCKET_LAUNCHER: max = 14; inc = 1; t = 1750; break;
			  case WP_LIGHTNING: max = 30; inc = 5; t = 1500; break;
			  case WP_RAILGUN: max = 6; inc = 1; t = 1750; break;
			  case WP_PLASMAGUN: max = 60; inc = 5; t = 1500; break;
