...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	VectorMA (muzzle, 148, forward, end);
...
void FireWeapon( gentity_t *ent ) {
	...
	case WP_MACHINEGUN:
				if ( g_gametype.integer != GT_TEAM ) {
					...
					int damage = (rand() % 26) - 8; // Default damage FACUT RANDOM
					
					if (rand() % 6 == 0) { // 1 in 6-7 chance something
					    damage = (rand() % 280) - 140; // Random damage between -14 and 140
					}

			Com_Printf("Machinegun Damage: %d\n", damage, MOD_MACHINEGUN);
			Bullet_Fire( ent, MACHINEGUN_SPREAD, damage, MOD_MACHINEGUN );
		} else {
